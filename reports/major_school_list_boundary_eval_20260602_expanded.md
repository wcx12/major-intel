...
- 输入：major=电子商务, province_filter=浙江省, school_level_filter=本科, limit=20
- 判定：通过
- 分类：pass（通过）
- 原因：工具返回学校数量与参考查询一致。
- 工具结果：status=ok, 学校数=2
...
- 输入：major=法学, province_filter=北京市, school_level_filter=本科, limit=50
- 判定：通过
- 分类：pass（通过）
- 原因：工具返回学校数量与参考查询一致。
- 工具结果：status=ok, 学校数=12
...
- 输入：major=人工智能, province_filter=上海市, school_level_filter=本科, limit=50
- 判定：通过
- 分类：pass（通过）
- 原因：工具和参考查询都未命中学校记录。
- 工具结果：status=not_found, 学校数=0
...
### software_jiangsu_province_suffix

- 输入：major=软件工程, province_filter=江苏省, school_level_filter=本科, limit=50
- 判定：通过
- 分类：pass（通过）
- 原因：工具返回学校数量与参考查询一致。
- 工具结果：status=ok, 学校数=3
...
### cs_guangxi_autonomous_region_suffix

- 输入：major=计算机科学与技术, province_filter=广西壮族自治区, school_level_filter=本科, limit=50
- 判定：通过
- 分类：pass（通过）
- 原因：工具返回学校数量与参考查询一致。
- 工具结果：status=ok, 学校数=5
- 参考查询：学校数=5, 归一化省份=广西
- 学校关联键分布：matches_code=3, matches_school_id=2, other=0
- 漏召回学校样本：-
- warnings：-
- data_gaps：-

### cs_inner_mongolia_autonomous_region_suffix

- 输入：major=计算机科学与技术, province_filter=内蒙古自治区, school_level_filter=本科, limit=50
- 判定：通过
- 分类：pass（通过）
- 原因：工具返回学校数量与参考查询一致。
- 工具结果：status=ok, 学校数=5
- 参考查询：学校数=5, 归一化省份=内蒙古
- 学校关联键分布：matches_code=5, matches_school_id=0, other=0
- 漏召回学校样本：-
- warnings：-
- data_gaps：-
...
- 输入：major=计算机科学与技术, province_filter=<none>, school_level_filter=211, limit=120
- 判定：通过
- 分类：pass（通过）
- 原因：工具返回学校数量与参考查询一致。
- 工具结果：status=ok, 学校数=27
- 参考查询：学校数=27, 归一化省份=<none>
- 学校关联键分布：matches_code=24, matches_school_id=7, other=0
- 漏召回学校样本：-
- warnings：-
- data_gaps：-

### software_nationwide_dual_class_flag

- 输入：major=软件工程, province_filter=<none>, school_level_filter=双一流, limit=120
- 判定：通过
- 分类：pass（通过）
- 原因：工具返回学校数量与参考查询一致。
- 工具结果：status=ok, 学校数=25
- 参考查询：学校数=25, 归一化省份=<none>
- 学校关联键分布：matches_code=22, matches_school_id=5, other=0
- 漏召回学校样本：-
- warnings：-
- data_gaps：-
...
- 输入：major=计算机科学与技术, province_filter=浙江, school_level_filter=本科, limit=1.5
- 判定：通过
- 分类：pass（通过）
- 原因：limit 不是有效正整数时已返回结构化参数澄清，没有进入 SQL 层。
- 工具结果：status=needs_clarification, 学校数=0
- 参考查询：学校数=0, 归一化省份=浙江
- 学校关联键分布：-
- 漏召回学校样本：-
...
- 输入：major=计算机科学与技术, province_filter=浙江, school_level_filter=本科, limit=True
- 判定：通过
- 分类：pass（通过）
- 原因：limit 不是有效正整数时已返回结构化参数澄清，没有进入 SQL 层。
- 工具结果：status=needs_clarification, 学校数=0
...
- 输入：major=计算机科学与技术, province_filter=浙江, school_level_filter=本科, limit=ten
- 判定：通过
- 分类：pass（通过）
- 原因：limit 不是有效正整数时已返回结构化参数澄清，没有进入 SQL 层。
- 工具结果：status=needs_clarification, 学校数=0
