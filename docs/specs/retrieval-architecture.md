...
## 第一版 SQL-first RAG 定义

第一版 RAG 不先做向量库、网页搜索或 PDF 抓取，而是把本地 MySQL 和正式 function call 工具作为 retriever。

这里的 RAG 含义是：

```text
用户自然语言问题
  -> agent 识别 intent 和 slots
  -> agent 选择一个或多个本地 function call
  -> function call 查询 MySQL 并返回结构化上下文
  -> agent 只能基于工具结果、scope_notes、warnings、data_gaps 组织回答
  -> 查不到就返回缺口并入队，不让模型凭记忆补事实
```

因此，第一版 SQL-first RAG 的检索源是：

- 27 个正式 function call。
- 本地 MySQL 中已经导入的学校、专业、录取、计划、专业组、就业摘要、市场样本、考公样本、转专业线索等表。
- `retrieval_cache` 中可复用的历史结构化结果。

第一版 SQL-first RAG 的上下文是每个工具返回的统一结构：

- `data`：结构化事实和样本。
- `scope_notes`：数据口径和适用范围。
- `warnings`：高风险误用提示。
- `data_gaps`：本地库暂时无法支撑的字段。
- `source_tables`：结果来自哪些本地表。

第一版不要求“搜全网”，而是要求“把本地库能回答的稳定答出来，把不能回答的稳定拒绝或标缺口”。后续联网 RAG 只消费缺口任务，用来补证据和写回数据库，不直接替代用户问答链路。
...
用户问题
  -> 问题类型识别
  -> 学校/专业/省份/年份/批次实体识别
  -> 按问题类型选择 function call
  -> function call 执行 MySQL 结构化查询
  -> 生成标准上下文包
  -> 判断缺失字段
  -> 生成回答或写入数据缺口队列
```
