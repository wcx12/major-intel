...
| `limit` | 否 | 返回条数上限。当前正数 limit 会下推到 SQL；`0` 和负数的参数校验仍是暂缓项。 |
...
| `needs_clarification` | 学校输入命中多个候选或简称高度歧义。 |
| `error` | 数据库查询或工具执行异常。 |
...
当前仍保留 3 个暂缓项，并在审计用例里持续暴露：

| case_id | 问题 | 当前处理 |
|---|---|---|
| `huada_alias` | “华大”简称危险，可能指向多所学校。 | 暂不修，后续统一处理学校简称歧义策略。 |
| `cupl_limit_0` | `limit=0` 没有提前返回结构化参数校验。 | 暂不修。 |
| `cupl_limit_negative` | `limit=-1` 可能进入 SQL 层或返回错误。 | 暂不修。 |

除上述暂缓项外，最新真实库审计基线为 36 个用例中 33 个通过、3 个失败、0 个需要复核。
