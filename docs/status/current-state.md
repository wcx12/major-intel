...
- 单元测试：`python -m unittest discover -s tests` 已通过 133 个测试；烟测 runner 现在会直接对齐 `RETRIEVAL_FUNCTION_NAMES`，避免以后新增工具但忘记补 smoke 用例。
- 烟测矩阵：`data/retrieval_smoke_cases.json` 已覆盖 27 个正式工具，展开后为 305 个真实库用例。
...
| 第一版 SQL-first RAG：学生提问时由 agent 选择 function call，查本地 SQL 并基于结构化上下文回答 | 第一版已完成 | 已有 27 个 function schema、工具层、`source_trace_lookup`、`data_gap_detection`、自然语言规则入口、DeepSeek tool-call agent、统一入口、查询缓存/日志表、`data_gap_queue` 入队和 `data_gap_evidence_tasks` 本地任务生成；27 工具真实库 smoke 矩阵已补齐并全量跑通，当前质量提示为 0 | 仍需继续人工抽测真实问题，发现更细的别名、口径和数据覆盖问题 |
