...
def _write_report(snapshot_path: Path, reports_dir: Path, profession_id: int) -> None:
    try:
        # The crawler can optionally render a markdown summary after fetching.
        # Import the report writer lazily so normal crawl/test paths do not pay
        # for report dependencies, and keep the implementation importable from
        # the package rather than the compatibility ``scripts`` namespace.
        from major_intel.reporting.rysxai_market_report import load_snapshot, write_markdown_report
