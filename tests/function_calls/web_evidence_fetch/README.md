...
默认 `source_policy=official_only`，只采纳学校官网、`.edu.cn`、省级考试院、政府域名、阳光高考等高可信来源。第三方页面默认不会被当成可核验证据；如果显式使用 `source_policy=any`，第三方页面也会抓取，但必须在回答中标注为线索。
...
- 验证 `source_policy=any` 可以抓取第三方页面，但置信度保持较低。
- 验证候选页抓取失败时返回 `partial + web_page_content`。
- 验证 registry schema、dispatcher、agent 自动 fallback 优先调用 `web_evidence_fetch`。

<!-- legacy-section: ## 5. 娴嬭瘯缁撴灉 -->
## 5. 测试结果

- 最近运行日期：2026-06-03
- 运行命令：`python -m pytest tests/test_web_evidence_fetch.py tests/test_web_evidence_search.py tests/test_retrieval_function_registry.py tests/test_deepseek_retrieval_agent.py tests/test_retrieval_tools.py -q`
- 运行结果：`125 passed in 1.19s`
...
- 真实 agent 结果：自动调用 `major_school_list -> web_evidence_fetch`；由于 `official_only` 未找到官方正文证据，最终没有再使用第三方摘要生成院校名单。
